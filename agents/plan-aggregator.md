...
Both header lines are mandatory -- the fix-plan is not valid output without them. The `**Original plan:**` line must ALWAYS reference the original (non-fix-plan) source plan, never the fix-plan you are currently generating from: a downstream skill parses this exact `**Original plan:**` token to recover the real plan a fix-plan traces back to.
