...
5. For every agent with `role: "test-author"`, its `owned_files` MUST be disjoint from the `owned_files` of every paired `impl` agent (an impl is paired with this test-author when the impl's `tests_to_satisfy` intersects the test-author's `owned_files`). Waves being different does NOT excuse the overlap -- a test-author that writes tests into files its paired impl owns is writing into modules nothing declares yet, so the red gate runs zero tests, exits 0, and the invalid-red rule silently skips the paired impl: a task lost while reporting BLOCKED. Exception: a test-author node declaring `inline_tests: true` (unit tests that must live inside the implementation file -- e.g. a `[[bin]]`-style target unreachable from an external test file) may share exactly ONE file with its paired impl; a second shared path still fails. On failure, fail validation and STOP, naming both agents and each shared path. (This enforces the analyzer role's "test files only" rule, which was previously unenforced.)
